...
`,
	...
	action(params, options) {
		...
		if (isV12()) {
			if (options['noconvert-eol'] && !options.build) {
				return Promise.reject(
					new ExpectedError(
						'The --noconvert-eol flag is only valid with --build.',
					),
				);
			}
			options.convertEol = !options['noconvert-eol'];
			delete options['noconvert-eol'];
		} else {
